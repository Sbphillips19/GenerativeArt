...
class Hexagon {
  ...
  split() {
    ...
    newHexagons.push(new Hexagon(this.center.x + random(-10, 10), this.center.y + random(-10, 10), newSize, this.hexColor));
    newHexagons.push(new Hexagon(this.center.x + random(-10, 10), this.center.y + random(-10, 10), newSize, this.hexColor));
    return newHexagons;
  }
}
...
function setup() {
  ...
  shapeType = random();
  ethSize = random();
  ethereumLogoColor = random();
  maxHexagons = int(random(150, 250));
  numHexagons = int(random(7, 10));
  splitSize = int(random(3, 5));
  numberHexagonsSplit = int(random(20, 25))
  ...
  for (let i = 0; i < numHexagons; i++) {
    let hexSize = random(minSize, maxSize);
...
function resetSketch() {
  ...
  for (let i = 0; i < numHexagons; i++) {
    let hexSize = random(minSize, maxSize);
    chains[0].addHexagon(new Hexagon(random(width), random(height), hexSize, optimismColor));
  }
}
...
function renderWave() {
  for (let x = 0; x < yvalues.length; x++) {
    push();
    if (ethSize < 0.3) {
      drawEthereumLogo(x * xspacing, 650 + yvalues[x], 30, 150);
    }
    else if (ethSize < 0.7) {
      drawEthereumLogo(x * xspacing, 600 + yvalues[x], 40, 150);
    } else {
      drawEthereumLogo(x * xspacing, 550 + yvalues[x], 50, 250);
    }
    pop();
  }
}
